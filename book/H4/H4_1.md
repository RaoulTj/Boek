...
\begin{align}
 ...
 y &= (C_1e + C_2x)e^{x} \qquad \text{met } C_{1,2} \in \mathbb{R}
\end{align}
```
````
...
\begin{align*}
 \dfrac{d^2y}{dx^2} +2 \dfrac{dy}{dx} +2 y = 0
\end{align*}
...
Invullen in de D.V. geeft:
\begin{align}
 (\lambda^2 Ce^{\lambda x}) +2 (\lambda Ce^{\lambda x}) +2 (Ce^{\lambda x}) &= 0 \\
 (\lambda^2 +2 \lambda +2 )\cdot Ce^{\lambda x}  &= 0
\end{align}
...
De oplossingen van de karakteristieke vergelijking zijn:
\begin{align}
  \lambda_{1,2} &= \dfrac{-2 \pm i\sqrt{4}}{2 \cdot 2}\\
  \lambda_{1,2} &= -\dfrac{2}{4} \pm i \dfrac{\sqrt{4}}{4} \\
 \lambda_{1,2} &= -\dfrac{1}{2} \pm i \dfrac{3}{4}
\end{align}

Hieruit volgt dat $p=-\dfrac{1}{2}$ en $q=\dfrac{3}{4}$
...
\begin{align}
 y = e^{-\frac{1}{2}x}\left(C_1\cos\left(\dfrac{3}{4}x\right) + C_2\sin \left( \dfrac{3}{4}x \right)\right) \qquad  \text{met } C_{1,2} \in \mathbb{R}
\end{align}
```
````