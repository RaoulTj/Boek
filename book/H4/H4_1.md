...
Dus de discriminant is,
\begin{align}
 D = (-4)^2 - 4 \cdot 2 \cdot +5 = 16 -20 =-4
\end{align}

De oplossingen van de karakteristieke vergelijking zijn:
\begin{align}
  \lambda_{1,2} &= \dfrac{4 \pm i\sqrt{4}}{2 \cdot 1}\\
  \lambda_{1,2} &= \dfrac{1}{2} \pm i \dfrac{\sqrt{4}}{2} \\
 \lambda_{1,2} &= \dfrac{1}{2} \pm i
\end{align}

Hieruit volgt dat $p=-\dfrac{1}{2}$ en $q=1$
...
\begin{align}
 y = e^{\frac{1}{2}x}\left(C_1\cos\left(x\right) + C_2\sin \left( x \right)\right) \qquad  \text{met } C_{1,2} \in \mathbb{R}
\end{align}
```
````
...
\begin{align*}
 2\dfrac{d^2y}{dx^2} +2 \dfrac{dy}{dx} +2 y = 0
\end{align*}
